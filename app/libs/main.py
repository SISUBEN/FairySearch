...
from app.i18n import _
from app.database.db_config import Config
...
class MainWindow(
    ...
    def onSearch(self) -> None:
        ...
        if len(keyword) > 0:
            ...
            if len(self.search_result) == 0:
                dialog = Dialog()
                logger.error(f"Search keyword: {keyword} does not exist")
    ...
    def converter(self, data: list) -> list:
        ...
        """
        return [
            {"cover": result[4] if len(result) > 4 else Config.DEFAULT_COVER, "title": result[1], "vid": result[0]}
            for result in data
        ]
